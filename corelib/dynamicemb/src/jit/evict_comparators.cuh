...
struct LexFreqTsComparator {
  ...
  __device__ __forceinline__ Rank rank(const uint64_t *scores, uint64_t /*cur_ts*/,
                                       double /*gamma*/) const {
...
struct UserFnComparator {
  ...
  __device__ __forceinline__ Rank rank(const uint64_t *scores, uint64_t cur_ts,
                                       double gamma) const {
    return user_score_fn(scores, cur_ts, static_cast<float>(gamma));
  }
};
