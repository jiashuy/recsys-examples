...
// ============================================================================
// segmented_unique_cuda_combine: sort-bucketed two-level (shared -> global)
// dedup.  Same result as segmented_unique_cuda, different method:
//   1. compute 32-bit combined_hash(key, table_id) per key
//   2. radix-sort by the TOP 8 bits of the hash (begin_bit=24,end_bit=32) so
//      identical keys (identical hash) cluster into the same block
//   3. per 1024-thread block: dedup in a shared hash table (size 2*blockDim),
//      warp-aggregating (key,table_id)+frequency first, then merge the block's
//      distinct keys into the global hash table once each.
// The sort makes within-block / within-warp duplicate clustering effective,
// so most duplicates collapse in shared memory instead of each probing global.
// ============================================================================

template <typename KeyType, typename Hasher>
__global__ void combine_hash_kernel(const KeyType *d_keys,
                                    const int64_t *d_table_ids, size_t num_keys,
                                    uint32_t *out_hash) {
  const size_t stride = blockDim.x * gridDim.x;
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < num_keys;
       i += stride) {
    uint32_t kh = Hasher::hash(d_keys[i]);
    uint32_t th = Hasher::hash(static_cast<uint32_t>(d_table_ids[i]));
    out_hash[i] = Hasher::hash_combine(kh, th);
  }
}

template <typename KeyType>
__global__ void
combine_gather_kernel(const KeyType *d_keys, const int64_t *d_table_ids,
                      const int64_t *d_freq_in, bool has_input,
                      const int64_t *sorted_idx, size_t num_keys,
                      KeyType *s_key, int32_t *s_tid, int64_t *s_freq,
                      int64_t *s_oidx) {
  const size_t stride = blockDim.x * gridDim.x;
  for (size_t j = blockIdx.x * blockDim.x + threadIdx.x; j < num_keys;
       j += stride) {
    int64_t i = sorted_idx[j];
    s_key[j] = d_keys[i];
    s_tid[j] = static_cast<int32_t>(d_table_ids[i]);
    s_freq[j] = has_input ? d_freq_in[i] : 1;
    s_oidx[j] = i;
  }
}

// Insert one block-distinct (key, table_id) into the GLOBAL composite table,
// accumulating freq; returns the per-table local unique index.  Same probe
// semantics as segmented_unique_kernel (this is called once per block-unique
// key, not once per occurrence).
template <typename KeyType, typename Hasher,
          KeyType empty_key = std::numeric_limits<KeyType>::max(),
          int64_t empty_val = std::numeric_limits<int64_t>::max()>
__device__ int32_t combine_global_insert(KeyType key, int tid, int64_t freq,
                                         bool enable_freq, KeyType *hash_keys,
                                         int64_t *hash_vals, size_t capacity,
                                         int64_t *table_counters,
                                         const int64_t *d_seg,
                                         KeyType *d_unique_keys,
                                         int64_t *frequency_counters) {
  uint32_t combined =
      Hasher::hash_combine(Hasher::hash(key), Hasher::hash(static_cast<uint32_t>(tid)));
  size_t hi = combined % capacity;
  for (size_t p = 0; p < capacity; ++p) {
    KeyType ek = hash_keys[hi];
    if (ek == empty_key) {
      KeyType old = atomicCAS(&hash_keys[hi], empty_key, key);
      if (old == empty_key) {
        int32_t local =
            static_cast<int32_t>(atomicAdd(&table_counters[tid], (int64_t)1));
        size_t op = static_cast<size_t>(d_seg[tid]) + local;
        d_unique_keys[op] = key;
        *reinterpret_cast<volatile int64_t *>(&hash_vals[hi]) =
            pack_table_val(tid, local);
        if (enable_freq)
          atomicAdd(&frequency_counters[op], freq);
        return local;
      } else if (old == key) {
        int64_t pv;
        do {
          pv = *reinterpret_cast<volatile int64_t *>(&hash_vals[hi]);
          __nanosleep(1);
        } while (pv == empty_val);
        if (unpack_table_id(pv) == tid) {
          int32_t local = unpack_local_idx(pv);
          if (enable_freq)
            atomicAdd(&frequency_counters[static_cast<size_t>(d_seg[tid]) + local],
                      freq);
          return local;
        }
      }
...
template <typename KeyType, typename Hasher,
          KeyType empty_key = std::numeric_limits<KeyType>::max(),
          int64_t empty_val = std::numeric_limits<int64_t>::max()>
__global__ void segmented_unique_combine_kernel(
    const KeyType *s_key, const int32_t *s_tid, const int64_t *s_freq,
    const int64_t *s_oidx, size_t num_keys, bool enable_freq,
    KeyType *hash_keys, int64_t *hash_vals, size_t capacity,
    int64_t *table_counters, const int64_t *d_seg, KeyType *d_unique_keys,
    int64_t *frequency_counters, int64_t *d_output_indices) {
  extern __shared__ unsigned char smem[];
  const int S = 2 * blockDim.x; // shared sub-table capacity (load factor <= 0.5)
  KeyType *skey = reinterpret_cast<KeyType *>(smem);
  int64_t *sfreq = reinterpret_cast<int64_t *>(smem + S * sizeof(KeyType));
  int32_t *stid = reinterpret_cast<int32_t *>(smem + S * (sizeof(KeyType) + sizeof(int64_t)));
  int32_t *sgidx = reinterpret_cast<int32_t *>(
      smem + S * (sizeof(KeyType) + sizeof(int64_t) + sizeof(int32_t)));
  ...
  if (active) {
    key = s_key[j];
    tid = s_tid[j];
    freq = enable_freq ? s_freq[j] : 0;
    oidx = s_oidx[j];

    // Warp-aggregate by (key, table_id).  After the sort, identical keys are
    // adjacent and usually share a warp, so only the run leader probes the
    // shared table (followers get the slot via shfl).  This is also REQUIRED
    // for correctness: if every lane independently CAS'd then spun waiting for
    // the owner to publish stid, same-warp owner/waiter lanes would deadlock.
    const unsigned amask = __activemask();
    const unsigned gk =
        __match_any_sync(amask, static_cast<unsigned long long>(key));
    const unsigned gt = __match_any_sync(amask, static_cast<unsigned>(tid));
    const unsigned grp = gk & gt;
    const int lane = threadIdx.x & 31;
    const int leader = __ffs(grp) - 1;

    int slot_found = -1;
    if (lane == leader) {
      uint32_t combined = Hasher::hash_combine(Hasher::hash(key),
                                               Hasher::hash(static_cast<uint32_t>(tid)));
      int slot = static_cast<int>(combined % static_cast<unsigned>(S));
      for (int p = 0; p < S; ++p) {
        KeyType cur = skey[slot];
        if (cur == empty_key) {
          KeyType old = atomicCAS(&skey[slot], empty_key, key);
          if (old == empty_key) {
            stid[slot] = tid;
            __threadfence_block();
            slot_found = slot;
            ...
            while (*reinterpret_cast<volatile int32_t *>(&stid[slot]) < 0) {
            }
            if (stid[slot] == tid) {
              slot_found = slot;
              break;
    ...
    my_slot = __shfl_sync(grp, slot_found, leader);
    if (enable_freq && my_slot >= 0)
      atomicAdd(&sfreq[my_slot], freq);
  }
  __syncthreads();

  // Merge the block's distinct keys into the global table (one insert each).
  // Each shared slot holds a unique (key,tid) for this block, so threads in a
  // warp here insert distinct global keys -> no same-key intra-warp spin.
  for (int s = threadIdx.x; s < S; s += blockDim.x) {
    if (skey[s] != empty_key) {
      sgidx[s] = combine_global_insert<KeyType, Hasher, empty_key, empty_val>(
          skey[s], stid[s], sfreq[s], enable_freq, hash_keys, hash_vals,
          capacity, table_counters, d_seg, d_unique_keys, frequency_counters);
    }
  }
  __syncthreads();

  if (active && my_slot >= 0)
    d_output_indices[oidx] = sgidx[my_slot];
}

std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor>
segmented_unique_cuda_combine(at::Tensor keys, at::Tensor segmented_range,
                              int64_t num_tables, at::Tensor input_frequencies) {
  cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
  const int64_t num_keys = keys.numel();
  const auto device = keys.device();
  ...
  TORCH_CHECK(segmented_range.numel() == num_tables + 1,
              "segmented_range must have num_tables+1 elements");
  TORCH_CHECK(num_keys < std::numeric_limits<int32_t>::max(),
              "num_keys must be < INT32_MAX");

  const bool enable_freq = input_frequencies.defined();
  const bool has_input = enable_freq && input_frequencies.numel() > 0;
  if (has_input)
    TORCH_CHECK(input_frequencies.numel() == num_keys,
                "input_frequencies must match keys length");

  auto i64 = at::TensorOptions().dtype(at::kLong).device(device);
  auto i32 = at::TensorOptions().dtype(at::kInt).device(device);

  if (num_keys == 0) {
    at::Tensor table_offsets = at::zeros({num_tables + 1}, i64);
    at::Tensor num_uniques = table_offsets.slice(0, num_tables, num_tables + 1);
    return std::make_tuple(num_uniques, at::empty({0}, keys.options()),
                           at::empty({0}, i64), table_offsets,
                           at::empty({0}, i64));
  }

  const int GBLK = 256;
  const int ggrid = sm * 8;
  const int64_t capacity = num_keys * 2;

  at::Tensor table_ids = at::empty({num_keys}, i64);
  expand_table_ids_kernel<<<ggrid, GBLK, 0, stream>>>(
      get_pointer<const int64_t>(segmented_range),
      get_pointer<int64_t>(table_ids), num_tables, num_keys);
  DEMB_CUDA_KERNEL_LAUNCH_CHECK();

  at::Tensor chash = at::empty({num_keys}, i32);
  at::Tensor chash_sorted = at::empty({num_keys}, i32);
  at::Tensor idx_in =
      at::arange(num_keys, i64);
  at::Tensor idx_sorted = at::empty({num_keys}, i64);
  at::Tensor s_key = at::empty({num_keys}, keys.options());
  at::Tensor s_tid = at::empty({num_keys}, i32);
  at::Tensor s_freq = at::empty({num_keys}, i64);
  at::Tensor s_oidx = at::empty({num_keys}, i64);

  at::Tensor hash_keys = at::empty({capacity}, keys.options());
  at::Tensor hash_vals = at::empty({capacity}, i64);
  at::Tensor table_counters = at::zeros({num_tables}, i64);
  at::Tensor partitioned_keys = at::empty({num_keys}, keys.options());
  at::Tensor output_indices = at::empty({num_keys}, i64);
  at::Tensor partitioned_freq =
      enable_freq ? at::zeros({num_keys}, i64) : at::empty({0}, i64);

  dispatch_key_type(key_dtype, [&]<typename KeyType>() {
    using Hasher = MurmurHash3_32<KeyType>;
    combine_hash_kernel<KeyType, Hasher><<<ggrid, GBLK, 0, stream>>>(
        get_pointer<const KeyType>(keys), get_pointer<const int64_t>(table_ids),
        num_keys, reinterpret_cast<uint32_t *>(get_pointer<int32_t>(chash)));
    DEMB_CUDA_KERNEL_LAUNCH_CHECK();

    // Sort by the LOW 16 bits of the 32-bit hash (begin_bit=0, end_bit=16):
    // finer buckets than the high byte, and aligned with the hash-table slot
    // (slot = hash % capacity, capacity ~ 2^21) so identical keys cluster and
    // consecutive blocks touch nearby global slots.
    auto *hin = reinterpret_cast<uint32_t *>(get_pointer<int32_t>(chash));
    auto *hout = reinterpret_cast<uint32_t *>(get_pointer<int32_t>(chash_sorted));
    auto *vin = get_pointer<int64_t>(idx_in);
    auto *vout = get_pointer<int64_t>(idx_sorted);
    size_t tmp_bytes = 0;
    cub::DeviceRadixSort::SortPairs(nullptr, tmp_bytes, hin, hout, vin, vout,
                                    num_keys, 0, 16, stream);
    at::Tensor tmp = at::empty({static_cast<int64_t>(tmp_bytes)},
                               at::TensorOptions().dtype(at::kByte).device(device));
    cub::DeviceRadixSort::SortPairs(tmp.data_ptr(), tmp_bytes, hin, hout, vin,
                                    vout, num_keys, 0, 16, stream);

    combine_gather_kernel<KeyType><<<ggrid, GBLK, 0, stream>>>(
        get_pointer<const KeyType>(keys), get_pointer<const int64_t>(table_ids),
        has_input ? get_pointer<const int64_t>(input_frequencies) : nullptr,
        has_input, get_pointer<const int64_t>(idx_sorted), num_keys,
        get_pointer<KeyType>(s_key), get_pointer<int32_t>(s_tid),
        get_pointer<int64_t>(s_freq), get_pointer<int64_t>(s_oidx));
    DEMB_CUDA_KERNEL_LAUNCH_CHECK();

    segmented_init_kernel<KeyType><<<ggrid, GBLK, 0, stream>>>(
        get_pointer<KeyType>(hash_keys), get_pointer<int64_t>(hash_vals),
        get_pointer<int64_t>(table_counters), capacity, num_tables);
    DEMB_CUDA_KERNEL_LAUNCH_CHECK();

    const int CBLK = 1024;
    const int cgrid = static_cast<int>((num_keys + CBLK - 1) / CBLK);
    const int S = 2 * CBLK;
    const size_t shmem =
        static_cast<size_t>(S) *
        (sizeof(KeyType) + sizeof(int64_t) + sizeof(int32_t) + sizeof(int32_t));
    auto kern = segmented_unique_combine_kernel<KeyType, Hasher>;
    cudaFuncSetAttribute(reinterpret_cast<const void *>(kern),
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         static_cast<int>(shmem));
    kern<<<cgrid, CBLK, shmem, stream>>>(
        get_pointer<const KeyType>(s_key), get_pointer<const int32_t>(s_tid),
        get_pointer<const int64_t>(s_freq), get_pointer<const int64_t>(s_oidx),
        num_keys, enable_freq, get_pointer<KeyType>(hash_keys),
        get_pointer<int64_t>(hash_vals), capacity,
        get_pointer<int64_t>(table_counters),
        get_pointer<const int64_t>(segmented_range),
        get_pointer<KeyType>(partitioned_keys),
        enable_freq ? get_pointer<int64_t>(partitioned_freq) : nullptr,
        get_pointer<int64_t>(output_indices));
    DEMB_CUDA_KERNEL_LAUNCH_CHECK();
  });

  // table_offsets = inclusive scan of table_counters (same as segmented_unique)
  at::Tensor table_offsets = at::zeros({num_tables + 1}, i64);
  size_t scan_bytes = 0;
  cub::DeviceScan::InclusiveSum(nullptr, scan_bytes,
                                ...
                                get_pointer<int64_t>(table_offsets) + 1,
                                num_tables, stream);
  at::Tensor scan_tmp = at::empty({static_cast<int64_t>(scan_bytes)},
                                  at::TensorOptions().dtype(at::kByte).device(device));
  cub::DeviceScan::InclusiveSum(scan_tmp.data_ptr(), scan_bytes,
                                get_pointer<int64_t>(table_counters),
                                get_pointer<int64_t>(table_offsets) + 1,
                                num_tables, stream);

  at::Tensor unique_keys = at::empty({num_keys}, keys.options());
  at::Tensor output_freq =
      enable_freq ? at::empty({num_keys}, i64) : at::empty({0}, i64);
  dispatch_key_type(key_dtype, [&]<typename KeyType>() {
    compact_keys_and_freq_kernel<KeyType><<<ggrid, GBLK, 0, stream>>>(
        get_pointer<const KeyType>(partitioned_keys),
        enable_freq ? get_pointer<const int64_t>(partitioned_freq) : nullptr,
        get_pointer<const int64_t>(segmented_range),
        get_pointer<const int64_t>(table_offsets), num_tables,
        get_pointer<KeyType>(unique_keys),
        enable_freq ? get_pointer<int64_t>(output_freq) : nullptr,
        get_pointer<const int64_t>(table_offsets) + num_tables);
    DEMB_CUDA_KERNEL_LAUNCH_CHECK();
  });

  adjust_output_indices_kernel<<<ggrid, GBLK, 0, stream>>>(
      get_pointer<const int64_t>(table_ids),
      get_pointer<const int64_t>(table_offsets),
      get_pointer<int64_t>(output_indices), num_keys);
  DEMB_CUDA_KERNEL_LAUNCH_CHECK();
...
Returns:
  ...
  m.def(
      "segmented_unique_cuda_combine",
      [](at::Tensor keys, at::Tensor segmented_range, int64_t num_tables,
         const c10::optional<at::Tensor> &input_frequencies) {
        at::Tensor freq_tensor;
        if (input_frequencies.has_value())
          freq_tensor = input_frequencies.value();
        return dyn_emb::segmented_unique_cuda_combine(keys, segmented_range,
                                                      num_tables, freq_tensor);
      },
      R"doc(
Same result as segmented_unique_cuda, computed via a sort-bucketed two-level
(shared-memory -> global) dedup: keys are radix-sorted by the top 8 bits of
their 32-bit combined hash so duplicates cluster, deduped in a per-block shared
hash table (warp-aggregated), then merged once each into the global table.
Returns the same 5-tuple as segmented_unique_cuda.
)doc",
      py::arg("keys"), py::arg("segmented_range"), py::arg("num_tables"),
      py::arg("input_frequencies") = py::none());
