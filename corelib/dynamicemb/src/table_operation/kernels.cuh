...
table_lookup_kernel(Table table, int64_t batch,
                    ...
                    typename ScorePolicy::Argument score_args,
  ...
  for (int64_t i = tid; i < batch; i += gridDim.x * blockDim.x) {
    ...
    ScoreType score = ScorePolicy::get(score_args, i);
    ...
    int step = 0;
    auto probe_res =
        bucket.probe<KernelTraits::ProbingGroupSize>(key, iter, step);
...
table_insert_kernel(Table table, int *__restrict__ bucket_sizes, int64_t batch,
  ...
  using ReduceResult = typename Bucket::ReduceResult;
  ...
  extern __shared__ uint8_t sm_buffer[];
  ...
  for (int64_t i = tid; i < batch; i += gridDim.x * blockDim.x) {
    ...
    while (step != bucket.capacity()) {
      ...
      if (probe_res == ProbeResult::Existed) {
        ...
        if (bucket.try_lock(iter, expected_key)) {
          ...
          // got the same slot.
        } else { // else: the key is evicted from the bucket(full), try to
                 // reintert by eviction including reclaimed key.
    ...
    while (probe_res != ProbeResult::Existed and
           probe_res != ProbeResult::Failed) {
      ScoreType compare_score =
          ScorePolicy::score_for_compare(score); /// TODO:score_for_compare
      KeyType evict_key = Bucket::EmptyKey;
      if constexpr (PipelinedReduction) {
        reduce_res = bucket.pipelined_reduce<ReductionGroupSize, BulkDim>(
            iter, evict_key, compare_score,
            sm_score_buffers + threadIdx.x / ReductionGroupSize *
                                   ReductionGroupSize * BufferDim);
      } else {
        static_assert(false);
      }
    ...
    if (probe_res == ProbeResult::Existed) {
      score = ScorePolicy::update(bucket.scores(iter), score);
      *bucket.score(iter) = Bucket::MaxScore; // to avoid dead lock when reduce
                                              // brings by small score
      ScorePolicy::set(score_args, i, score);
      if (output_scores) {
