...
template <typename grad_t, typename emb_t> struct OptimizierInputV3 {
  const grad_t* grad_;
  const emb_t* emb_;
  emb_t* table_vector_;
  const uint32_t dim;
  ...
  DEVICE_INLINE emb_t* state_input_ptr() const {
    return table_vector_ + dim;
  }

  DEVICE_INLINE emb_t* state_output_ptr() const {
    return output_buffer + dim;
  }
};

template <typename grad_t, typename emb_t, int kWarpSize = 32>
struct SgdVecOptimizerV3 {
  const float lr;

  template <typename OptimizierInput>
  DEVICE_INLINE void update4(
      const OptimizierInput &input) {
...
struct AdamVecOptimizerV3 {
  ...
  DEVICE_INLINE void update4(
    ...
    for (int i = 0; VecSize * (kWarpSize * i + lane_id) < input.dim; ++i) {
      ...
      weight_vec.load(input.emb_input_ptr() + idx4);
      if (input.state_initialized()) {
        m_vec.load(input.state_input_ptr() + idx4); // m_ptr
        v_vec.load(input.state_input_ptr() + input.dim + idx4); // v_ptr
      } else {
        m_vec.reset();
        v_vec.reset();
      }
      ...
      // update m and v
      {
        ...
        grad_vec.load(input.grad_ptr() + idx4);
        {
          ...
          m_vec.val.z = beta1 * m_vec.val.z + (1.0f - beta1) * grad_vec.val.z;
          m_vec.val.w = beta1 * m_vec.val.w + (1.0f - beta1) * grad_vec.val.w;
          m_vec.store(input.state_output_ptr() + idx4);
        }

        {
          ...
          v_vec.val.w = beta2 * v_vec.val.w +
                        (1.0f - beta2) * grad_vec.val.w * grad_vec.val.w;
          v_vec.store(input.state_output_ptr() + input.dim + idx4);
        }
      }
  ...
  DEVICE_INLINE void update(
    ...
    for (int i = threadIdx.x; i < input.dim; i += blockDim.x) {
      float tmp_grad = TypeConvertFunc<float, grad_t>::convert(input.grad_ptr()[i]);
      float tmp_m = input.state_initialized() ? TypeConvertFunc<float, emb_t>::convert(input.state_input_ptr()[i]) : 0.0f;
      float tmp_v = input.state_initialized() ? TypeConvertFunc<float, emb_t>::convert((input.state_input_ptr() + input.dim)[i]) : 0.0f;
      float tmp_weight = TypeConvertFunc<float, emb_t>::convert(input.emb_input_ptr()[i]);
      ...
      tmp_weight -= lr * ((tmp_mhat / (sqrtf(tmp_vhat) + eps)) +
                          weight_decay * tmp_weight);
      input.emb_output_ptr()[i] = TypeConvertFunc<emb_t, float>::convert(tmp_weight);
      input.state_output_ptr()[i] = TypeConvertFunc<emb_t, float>::convert(tmp_m);
      (input.state_output_ptr() + input.dim)[i] = TypeConvertFunc<emb_t, float>::convert(tmp_v);
    }
  }
};

template <typename grad_t, typename emb_t ,int kWarpSize = 32>
struct AdaGradVecOptimizerV3 {
  const float lr;
  const float eps;
  const float initial_accumulator;

  template <typename OptimizierInput>
  DEVICE_INLINE void update4(const OptimizierInput &input) {
    ...
    for (int i = 0; VecSize * kWarpSize * i + VecSize * lane_id < input.dim; ++i) {
        ...
        weight_vec.load(input.emb_input_ptr() + idx4);
        if (input.state_initialized()) {
          gt_vec.load(input.state_input_ptr() + idx4);
        } else {
          gt_vec.reset(initial_accumulator);
        }
        ...
        grad_vec.load(input.grad_ptr() + idx4);
        {
            ...
            gt_vec.val.z += grad_vec.val.z * grad_vec.val.z;
            gt_vec.val.w += grad_vec.val.w * grad_vec.val.w;
            gt_vec.store(input.state_output_ptr() + idx4);
        }
  ...
  DEVICE_INLINE void update(
    ...
    for (int i = threadIdx.x; i < input.dim; i+=blockDim.x) {
      float tmp_grad =  TypeConvertFunc<float, grad_t>::convert(input.grad_ptr()[i]);
      float tmp_gt = input.state_initialized() ? TypeConvertFunc<float, emb_t>::convert(input.state_input_ptr()[i]) : initial_accumulator;
      float tmp_weight = TypeConvertFunc<float, emb_t>::convert(input.emb_input_ptr()[i]);
      ...
      tmp_weight -= tmp_grad;
      input.emb_output_ptr()[i] = TypeConvertFunc<emb_t, float>::convert(tmp_weight);
      input.state_output_ptr()[i] = TypeConvertFunc<emb_t, float>::convert(tmp_gt);
    }
  }
};
...
template <
  ...
  typename emb_t,
  int kWarpSize = 32>
struct RowWiseAdaGradVecOptimizerV3 {
  const float lr;
  const float eps;
  const float initial_accumulator;
  ...
  DEVICE_INLINE void update4(const OptimizierInput& input) {
    ...
    float tmp_gt = input.state_initialized() ? TypeConvertFunc<float, emb_t>::convert(*(input.state_input_ptr())) : initial_accumulator;
    float tmp_g_pow = 0;
    ///TODO: vectorize
    for (int i = lane_id; i < input.dim; i += kWarpSize) {
      float tmp_g = TypeConvertFunc<float, grad_t>::convert(input.grad_ptr()[i]);
    ...
    if (lane_id == 0) {
      *(input.state_output_ptr()) = TypeConvertFunc<emb_t, float>::convert(tmp_gt);
    }
  ...
  template <typename OptimizierInput>
  DEVICE_INLINE void update(const OptimizierInput& input) {
...
DEVICE_INLINE bool found1(int id) {
  return founds1[id];
}

DEVICE_INLINE T* result1(int id) {
  return vals1[id];
}

DEVICE_INLINE int stage2_id(int id) {
  return missed_ids[id];
}

DEVICE_INLINE bool found2(int id) {
  return founds2[id];
}

DEVICE_INLINE T* result2(int id) {
  return vals2[id];
}
...
template <typename grad_t, typename emb_t, typename OptimizerFunc>
__global__ void update4_kernel_v3(
    ...
    TwoStageFoundResult<emb_t> found_res) {
  constexpr int kWarpSize = 32;
  const int warp_num_per_block = blockDim.x / kWarpSize;
  const int warp_id_in_block = threadIdx.x / kWarpSize;
  ...
  for (uint32_t ev_id = warp_num_per_block * blockIdx.x + warp_id_in_block;
    ...
    bool found = found_res.found1(ev_id);
    auto val_ptr = found_res.result1(ev_id);
    if (not found) {
      auto stage2_id = found_res.stage2_id(ev_id);
      found = found_res.found2(stage2_id);
      val_ptr = found_res.result2(stage2_id);
    }
...
template <typename grad_t, typename emb_t, typename OptimizerFunc>
__global__ void update_kernel_v3(
    const uint32_t num_keys, 
    const uint32_t dim,
    const uint32_t optstate_dim,
    const grad_t *grad_evs,
