...
from dynamicemb.types import MemoryType
...
class ScoreArg:
    ...
    policy: ScorePolicy  # How to set the new score
    is_return: bool  # Whether return the new score, if true will overwrite the `value`
...
class ScoredHashTable(abc.ABC):
    ...
    The value of the table is the ID of each key in the table， which is read-only.
    """
    ...
    @property
    def value_type(self) -> torch.dtype:
        """
    ...
    def insert(
        ...
        Values is output buffer if provided.
        """
...
class GroupedScoredHashTable(abc.ABC):
    ...
    key_type, value_type, offset_type, score_specs, result_type are the same for tables in the same group.
    """
...
class LinearBucketTable(ScoredHashTable):
    def __init__(
        ...
        accepted_score_types = {torch.uint64, torch.uint32}
        self.score_types_ = []
        self.score_names_ = []
        for score_spec in self.score_specs_:
            assert (
                score_spec.dtype in accepted_score_types
            ), "Only accept 64 and 32 bits unsigned integer as score's type."
    ...
    def _parse_scores(
        ...
        policies = [ScorePolicy.CONST.value for _ in self.score_names_]
        ...
        for score in scores:
            ...
            policies[index] = score.policy.value
    ...
    def insert_and_evict(
        ...
        batch = keys.numel()
        num_evicted = torch.empty(1, dtype=torch.int64, device=keys.device)
        evicted_keys = torch.empty(batch, dtype=self.key_type_, device=keys.device)
        evicted_values = torch.empty(batch, dtype=self.value_type, device=keys.device)
        evicted_scores_list = [
