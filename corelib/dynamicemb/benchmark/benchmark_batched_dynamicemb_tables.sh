    ...
    for optimizer_type in "${optimizer_types[@]}"; do
      ...
      for embedding_dim in "${embedding_dims[@]}"; do
        ...
        for alpha in "${alphas[@]}"; do
          ...
          nsys profile  -s none -t cuda,nvtx,osrt,mpi,ucx -f true -o de_and_tr-10.20.qdrep -c cudaProfilerApi  --cpuctxsw none --cuda-flush-interval 100 --capture-range-end=stop --cuda-graph-trace=node \
          torchrun --nnodes 1 --nproc_per_node 1 \
              ./benchmark/benchmark_batched_dynamicemb_tables.py  \
                --batch_size $batch_size \
                --num_embeddings_per_feature $capacity \
