      ...
      for optimizer_type in "${optimizer_types[@]}"; do
        for embedding_dim in "${embedding_dims[@]}"; do
          # for cache_algorithm in "${cache_algorithms[@]}"; do
            ...
            # nsys profile  -s none -t cuda,nvtx,osrt,mpi,ucx -f true -o dynamicemb$batch_size-$optimizer_type-vec.qdrep -c cudaProfilerApi  --cpuctxsw none --cuda-flush-interval 100 --capture-range-end=stop --cuda-graph-trace=node \
